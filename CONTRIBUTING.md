...
Kmpkg is a community driven effort to build a productive and robust ecosystem of native libraries - your contributions are invaluable!

## Issues

The easiest way to contribute is by reporting issues with either `kmpkg.exe` or an existing package on [GitHub](https://github.com/Microsoft/kmpkg). When reporting an issue with `kmpkg.exe`, make sure to clearly state:
...
- The steps to reproduce: "I run 'kmpkg list'"
...
- Prefer to use the `kmpkg_xyz` functions over raw `execute_command` calls. This makes long term maintenance easier when new features (such as custom compiler flags or generators) are added.
