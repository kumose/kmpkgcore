...
Kmpkg 是一个尝试由社区驱动，旨在构建一个多产的、健壮的本地库生态系统 - 您的贡献价值不可估量！

## 报告问题

最简单的方法是通过 `kmpkg.exe` 或 [GitHub](https://github.com/Microsoft/kmpkg) 报告已有的包的问题。 当报告 `kmpkg.exe` 的问题时，确保清楚地说明:
...
- 相比原始的 `execute_command` 调用，尽量改为通过 `kmpkg_xyz` 函数实现。这使得在添加新特性(如自定义编译器标志或生成器)时更容易进行长期维护。
