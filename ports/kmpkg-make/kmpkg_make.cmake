# Be aware of https://github.com/microsoft/kmpkg/pull/31228
include_guard(GLOBAL)
include("${CMAKE_CURRENT_LIST_DIR}/kmpkg_make_common.cmake")

function(kmpkg_run_shell)
    ...
    z_kmpkg_required_args(SHELL WORKING_DIRECTORY COMMAND LOGNAME)
    ...
    kmpkg_list(JOIN arg_COMMAND " " arg_COMMAND)
...
function(kmpkg_run_shell_as_build)
    cmake_parse_arguments(PARSE_ARGV 0 arg
        "" 
        "WORKING_DIRECTORY;LOGNAME"
...
function(kmpkg_run_autoreconf shell_cmd work_dir)
    ...
    if(NOT ACLOCAL)
        ...
        set(aclocal_check_dir "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/kmpkg-aclocal")
        file(REMOVE_RECURSE "${aclocal_check_dir}")
        file(MAKE_DIRECTORY "${aclocal_check_dir}")
        file(COPY_FILE "${CURRENT_HOST_INSTALLED_DIR}/share/kmpkg-make/configure.ac" "${aclocal_check_dir}/configure.ac")
        kmpkg_run_shell(
            SHELL ${shell_cmd}
...
function(kmpkg_make_get_shell out_var)
    set(shell_options "")
    if(CMAKE_HOST_WIN32)
        if(NOT DEFINED KMPKG_MAKE_ACQUIRE_MSYS OR KMPKG_MAKE_ACQUIRE_MSYS)
            kmpkg_make_setup_win_msys(msys_root)
            set(shell_options --noprofile --norc --debug)
            set(shell_cmd "${msys_root}/usr/bin/bash.exe")
        else()
            message(STATUS "Not acquiring msys, reason: KMPKG_MAKE_ACQUIRE_MSYS=${KMPKG_MAKE_ACQUIRE_MSYS}")
        endif()
    endif()
...
function(z_kmpkg_make_determine_target_triplet out)
    ...
    z_kmpkg_make_determine_target_arch(TARGET_ARCH)
    z_kmpkg_make_determine_host_arch(BUILD_ARCH)
    set(output "")
    if(KMPKG_MAKE_BUILD_TRIPLET MATCHES "--host=([^;]*)")
    ...
    elseif("${arg_COMPILER_NAME}" MATCHES "([^/]+)-(gcc|clang)(-[0-9]+)?$")
        ...
        set(output "${CMAKE_MATCH_1}")
    elseif(NOT KMPKG_CROSSCOMPILING AND KMPKG_MAKE_BUILD_TRIPLET MATCHES "--build=([^;]+)")
        set(output "${CMAKE_MATCH_1}")
    elseif(NOT "${TARGET_ARCH}" STREQUAL "${BUILD_ARCH}")
        message(${Z_KMPKG_BACKCOMPAT_MESSAGE_LEVEL}
            ...
            "You can set the KMPKG_MAKE_HOST_TRIPLET variable for ${TARGET_TRIPLET}."
        )
    endif()
    set("${out}" "${output}" PARENT_SCOPE)
endfunction()

function(z_kmpkg_make_get_configure_triplets out)
    cmake_parse_arguments(PARSE_ARGV 1 arg
        ""
        "COMPILER_NAME"
        ""
    ...
    set(output "${KMPKG_MAKE_BUILD_TRIPLET}")
    if(NOT output MATCHES "--host")
        z_kmpkg_make_determine_target_triplet(host_opt_triplet COMPILER_NAME "${arg_COMPILER_NAME}")
        if(host_opt_triplet)
            list(APPEND output "--host=${host_opt_triplet}")
        endif()
    endif()
    if(output MATCHES "--host" AND NOT output MATCHES "--build")
        file(STRINGS "${CURRENT_HOST_INSTALLED_DIR}/share/kmpkg-make/build_opt_triplet.txt" build_opt_triplet LIMIT_COUNT 1)
...
function(z_kmpkg_make_prepare_env config)
    cmake_parse_arguments(PARSE_ARGV 1 arg
        "ADD_BIN_TO_PATH"
        ""
        ""
    ...
    if(ARFLAGS_${config} AND NOT (arg_USE_WRAPPERS AND KMPKG_TARGET_IS_WINDOWS))
    ...
    kmpkg_list(APPEND lib_env_vars LIB LIBPATH LIBRARY_PATH)
    foreach(lib_env_var IN LISTS lib_env_vars)
        if(EXISTS "${CURRENT_INSTALLED_DIR}${path_suffix_${config}}/lib")
            kmpkg_host_path_list(PREPEND ENV{${lib_env_var}} "${CURRENT_INSTALLED_DIR}${path_suffix_${config}}/lib")
        endif()
        if(EXISTS "${CURRENT_INSTALLED_DIR}${path_suffix_${config}}/lib/manual-link")
            kmpkg_host_path_list(PREPEND ENV{${lib_env_var}} "${CURRENT_INSTALLED_DIR}${path_suffix_${config}}/lib/manual-link")
        endif()
    endforeach()
endfunction()

function(z_kmpkg_make_restore_env)
    # Only variables which are inspected in kmpkg_make_prepare_env need to be restored here.
    # Rest is restored add the end of configure. 
    kmpkg_restore_env_variables(VARS 
         LIBRARY_PATH LIB LIBPATH
         PATH
    )
endfunction()

function(kmpkg_make_run_configure)
    cmake_parse_arguments(PARSE_ARGV 0 arg
        "ADD_BIN_TO_PATH" 
    ...
    z_kmpkg_unparsed_args(FATAL_ERROR)
    z_kmpkg_required_args(SHELL CONFIG WORKING_DIRECTORY CONFIGURE_PATH)

    kmpkg_prepare_pkgconfig("${arg_CONFIG}")

    set(prepare_env_opts "")

    z_kmpkg_make_prepare_env("${arg_CONFIG}" ${prepare_env_opts})
    ...
    endforeach()
    kmpkg_list(JOIN tmp " " "arg_OPTIONS")
