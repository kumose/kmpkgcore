...
function(kmpkg_make_configure)
    ...
    if(DEFINED KMPKG_MAKE_CONFIGURE_OPTIONS)
        list(APPEND arg_OPTIONS ${KMPKG_MAKE_CONFIGURE_OPTIONS})
    endif()
    if(DEFINED KMPKG_MAKE_CONFIGURE_OPTIONS_RELEASE)
        list(APPEND arg_OPTIONS_RELEASE ${KMPKG_MAKE_CONFIGURE_OPTIONS_RELEASE})
    endif()
    if(DEFINED KMPKG_MAKE_CONFIGURE_OPTIONS_DEBUG)
        list(APPEND arg_OPTIONS_DEBUG ${KMPKG_MAKE_CONFIGURE_OPTIONS_DEBUG})
    endif()

    set(src_dir "${arg_SOURCE_PATH}")

    z_kmpkg_warn_path_with_spaces()
    ...
    set(Z_KMPKG_MAKE_GET_CMAKE_VARS_OPTS "ADDITIONAL_LANGUAGES;${arg_LANGUAGES}" CACHE INTERNAL "")
    ...
    z_kmpkg_set_global_property(make_prepare_flags_opts "${prepare_flags_opts}")
    z_kmpkg_make_prepare_flags(${prepare_flags_opts} ${escaping} C_COMPILER_NAME ccname FRONTEND_VARIANT_OUT frontend)

    z_kmpkg_make_get_configure_triplets(BUILD_TRIPLET COMPILER_NAME "${ccname}")
    ...
    if(arg_AUTORECONF)
      kmpkg_run_autoreconf("${shell_cmd}" "${src_dir}")
    endif()
    ...
    kmpkg_backup_env_variables(VARS 
        ${cm_FLAGS}
    # General backup
        PATH
    # Used by gcc/linux
    ...
    foreach(config IN LISTS buildtypes)
        ...
        z_kmpkg_make_prepare_programs(configure_env ${prepare_flags_opts} CONFIG "${configup}" BUILD_TRIPLET "${BUILD_TRIPLET}")

        set(opts "")
        if(NOT arg_DISABLE_DEFAULT_OPTIONS)
          z_kmpkg_make_default_path_and_configure_options(opts AUTOMAKE CONFIG "${configup}")
        endif()
        ...
        kmpkg_make_run_configure(SHELL
                                    "${shell_cmd}"
                                 CONFIG
                                    "${configup}"
                                 CONFIGURE_ENV
