...
function(kmpkg_cmake_config_fixup)
    ...
    if(DEFINED arg_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR "kmpkg_cmake_config_fixup was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}")
    endif()
    ...
    string(REPLACE "." "\\." EXECUTABLE_SUFFIX "${KMPKG_TARGET_EXECUTABLE_SUFFIX}")
    ...
    if(NOT arg_CONFIG_PATH STREQUAL "share/${arg_PACKAGE_NAME}")
        if(arg_CONFIG_PATH STREQUAL "share")
            set(arg_CONFIG_PATH z_kmpkg_share)
    ...
    if(NOT DEFINED Z_KMPKG_CMAKE_CONFIG_ALREADY_FIXED_UP)
        kmpkg_list(SET Z_KMPKG_CMAKE_CONFIG_ALREADY_FIXED_UP)
    endif()
    foreach(already_fixed_up IN LISTS Z_KMPKG_CMAKE_CONFIG_ALREADY_FIXED_UP)
        kmpkg_list(REMOVE_ITEM main_cmakes "${already_fixed_up}")
    endforeach()
    kmpkg_list(APPEND Z_KMPKG_CMAKE_CONFIG_ALREADY_FIXED_UP ${main_cmakes})
    set(Z_KMPKG_CMAKE_CONFIG_ALREADY_FIXED_UP "${Z_KMPKG_CMAKE_CONFIG_ALREADY_FIXED_UP}" CACHE INTERNAL "")
...
[[get_filename_component(PACKAGE_PREFIX_DIR "${CMAKE_CURRENT_LIST_DIR}/../../" ABSOLUTE)]]
        ...
        elseif(NOT EXISTS "${debug_cmake}")
            ...
            while(remainder)
                z_kmpkg_cmake_config_fixup_match_command("${remainder}" "set_target_properties(" matched_command remainder)
                ...
                z_kmpkg_cmake_config_fixup_match_command("${debug_contents}" "set_target_properties(${target} " debug_command unused)
                ...
                z_kmpkg_cmake_config_fixup_merge(merged_libs release_libs debug_libs)
        ...
        #This happens if kmpkg built libraries are directly linked to a target instead of using
        #an imported target.
        string(REPLACE "${CURRENT_INSTALLED_DIR}" [[${KMPKG_IMPORT_PREFIX}]] contents "${contents}")
        file(TO_CMAKE_PATH "${CURRENT_PACKAGES_DIR}" cmake_current_packages_dir)
        string(REPLACE "${cmake_current_packages_dir}" [[${KMPKG_IMPORT_PREFIX}]] contents "${contents}")
        # If ${KMPKG_IMPORT_PREFIX} was actually used, inject a definition of it:
        string(FIND "${contents}" [[${KMPKG_IMPORT_PREFIX}]] index)
        if (NOT index STREQUAL "-1")
            ...
            string(PREPEND contents "get_filename_component(KMPKG_IMPORT_PREFIX \"\${CMAKE_CURRENT_LIST_DIR}\/${relative}\" ABSOLUTE)\n")
        endif()
...
function(z_kmpkg_cmake_config_fixup_match_command haystack needle out_match out_remainder)
    set(match "")
    set(remainder "")
...
function(z_kmpkg_cmake_config_fixup_merge out_var release_var debug_var)
    set(release_libs "KMPKG;${${release_var}}")
    ...
    set(debug_libs "KMPKG;${${debug_var}}")
