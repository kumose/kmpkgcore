...
elseif(KMPKG_TARGET_ARCHITECTURE STREQUAL "arm64")
    ...
    elseif(KMPKG_DETECTED_CMAKE_C_COMPILER_ID MATCHES "Clang")
        set(OPENSSL_ARCH VC-CLANG-WIN64-CLANGASM-ARM)
    else()
        set(OPENSSL_ARCH VC-WIN64-CLANGASM-ARM)
    endif()
else()
    message(FATAL_ERROR "Unsupported target architecture: ${KMPKG_TARGET_ARCHITECTURE}")
endif()
...
if(KMPKG_CONCURRENCY GREATER "1")
    kmpkg_list(APPEND CONFIGURE_OPTIONS no-makedepend)
endif()
...
if (KMPKG_DETECTED_CMAKE_C_COMPILER_ID MATCHES "Clang" OR KMPKG_LIBRARY_LINKAGE STREQUAL "static")
    set(OPENSSL_BUILD_MAKES_PDBS OFF)
endif()

cmake_path(NATIVE_PATH KMPKG_DETECTED_CMAKE_C_COMPILER NORMALIZE cc)
if(OPENSSL_ARCH MATCHES "CLANG")
    kmpkg_find_acquire_program(CLANG)
    cmake_path(GET CLANG PARENT_PATH clang_path)
    kmpkg_add_to_path("${clang_path}")
    if(KMPKG_DETECTED_CMAKE_C_COMPILER_ID MATCHES "Clang")
        string(APPEND KMPKG_COMBINED_C_FLAGS_DEBUG " --target=aarch64-win32-msvc")
        string(APPEND KMPKG_COMBINED_C_FLAGS_RELEASE " --target=aarch64-win32-msvc")
    endif()
endif()
if(OPENSSL_ARCH MATCHES "CLANGASM")
    kmpkg_list(APPEND CONFIGURE_OPTIONS "ASFLAGS=--target=aarch64-win32-msvc")
else()
    kmpkg_find_acquire_program(NASM)
    cmake_path(NATIVE_PATH NASM NORMALIZE as)
    cmake_path(GET NASM PARENT_PATH nasm_path)
    kmpkg_add_to_path("${nasm_path}") # Needed by Configure
endif()

cmake_path(NATIVE_PATH KMPKG_DETECTED_CMAKE_AR NORMALIZE ar)
cmake_path(NATIVE_PATH KMPKG_DETECTED_CMAKE_LINKER NORMALIZE ld)
...
kmpkg_build_nmake(
    ...
    PRERUN_SHELL_RELEASE "${PERL}" Configure
        ...
        "AS=${as}"
        "CC=${cc}"
        "CFLAGS=${KMPKG_COMBINED_C_FLAGS_RELEASE}"
        "AR=${ar}"
        "ARFLAGS=${KMPKG_COMBINED_STATIC_LINKER_FLAGS_RELEASE}"
        "LD=${ld}"
        "LDFLAGS=${KMPKG_COMBINED_SHARED_LINKER_FLAGS_RELEASE}"
    PRERUN_SHELL_DEBUG "${PERL}" Configure
        ...
        "AS=${as}"
        "CC=${cc}"
        "CFLAGS=${KMPKG_COMBINED_C_FLAGS_DEBUG}"
        "AR=${ar}"
        "ARFLAGS=${KMPKG_COMBINED_STATIC_LINKER_FLAGS_DEBUG}"
        "LD=${ld}"
        "LDFLAGS=${KMPKG_COMBINED_SHARED_LINKER_FLAGS_DEBUG}"
