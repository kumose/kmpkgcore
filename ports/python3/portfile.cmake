...
if(KMPKG_TARGET_IS_WINDOWS)
    string(COMPARE EQUAL "${KMPKG_LIBRARY_LINKAGE}" "dynamic" PYTHON_ALLOW_EXTENSIONS)
    if(PYTHON_HAS_EXTENSIONS AND NOT PYTHON_ALLOW_EXTENSIONS)
        # This should never be reached due to kmpkg_check_linkage above
    ...
    kmpkg_get_windows_sdk(WINSDK_VERSION)
...
if(KMPKG_TARGET_IS_WINDOWS)
    ...
    if(PYTHON_HAS_EXTENSIONS)
        ...
        x_kmpkg_pkgconfig_get_modules(PREFIX PC_SQLITE3 MODULES sqlite3 LIBRARIES USE_MSVC_SYNTAX_ON_WINDOWS)
    ...
    configure_file("${CMAKE_CURRENT_LIST_DIR}/python_kmpkg.props.in" "${SOURCE_PATH}/PCbuild/python_kmpkg.props")
    ...
    list(APPEND KMPKG_CMAKE_CONFIGURE_OPTIONS "-DKMPKG_SET_CHARSET_FLAG=OFF")
    if(PYTHON_HAS_EXTENSIONS)
        set(OPTIONS
            ...
            "/p:ForceImportBeforeCppTargets=${SOURCE_PATH}/PCbuild/python_kmpkg.props"
        )
    else()
        set(OPTIONS
    ...
    if(KMPKG_LIBRARY_LINKAGE STREQUAL "dynamic")
        list(APPEND OPTIONS "/p:_KmpkgPythonLinkage=DynamicLibrary")
    else()
        list(APPEND OPTIONS "/p:_KmpkgPythonLinkage=StaticLibrary")
    endif()
    ...
    if(KMPKG_CROSSCOMPILING)
        kmpkg_add_to_path("${CURRENT_HOST_INSTALLED_DIR}/manual-tools/${PORT}")
    endif()
    ...
    # The extension modules must be placed in the DLLs directory, so we can't use kmpkg_copy_tools()
    if(PYTHON_HAS_EXTENSIONS)
        ...
        kmpkg_copy_tool_dependencies("${CURRENT_PACKAGES_DIR}/tools/${PORT}/DLLs")
    ...
    if(KMPKG_LIBRARY_LINKAGE STREQUAL "dynamic" AND NOT KMPKG_BUILD_TYPE)
    ...
    if(KMPKG_TARGET_IS_OSX OR KMPKG_TARGET_IS_BSD)
    ...
    if(KMPKG_TARGET_IS_ANDROID)
        ...
        list(APPEND KMPKG_CMAKE_CONFIGURE_OPTIONS "-DANDROID_NO_UNDEFINED=OFF")
        if(KMPKG_CROSSCOMPILING)
    ...
    kmpkg_make_configure(
        ...
        OPTIONS_DEBUG
            "--with-pydebug"
            "kmpkg_rpath=${CURRENT_INSTALLED_DIR}/debug/lib"
        OPTIONS_RELEASE
            "kmpkg_rpath=${CURRENT_INSTALLED_DIR}/lib"
    )
    kmpkg_make_install(TARGETS altinstall)
...
function(_generate_finder)
    ...
    configure_file(
        ...
        "${CURRENT_PACKAGES_DIR}/share/${PythonFinder_DIRECTORY}/kmpkg-cmake-wrapper.cmake"
        @ONLY
    )
endfunction()
...
if (NOT KMPKG_TARGET_IS_WINDOWS)
    function(replace_dirs_in_config_file python_config_file)
        kmpkg_replace_string("${python_config_file}" "${CURRENT_INSTALLED_DIR}" "' + _base + '")
        kmpkg_replace_string("${python_config_file}" "${CURRENT_HOST_INSTALLED_DIR}" "' + _base + '/../${HOST_TRIPLET}" IGNORE_UNCHANGED)
        kmpkg_replace_string("${python_config_file}" "${CURRENT_PACKAGES_DIR}" "' + _base + '")
        kmpkg_replace_string("${python_config_file}" "${CURRENT_BUILDTREES_DIR}" "not/existing")
    endfunction()
    ...
    if(NOT DEFINED KMPKG_BUILD_TYPE OR KMPKG_BUILD_TYPE STREQUAL "release")
        ...
        kmpkg_replace_string("${python_config_file}" "# system configuration generated and used by the sysconfig module" "# system configuration generated and used by the sysconfig module\nimport os\n_base = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))\n")
...
block()
  ...
  kmpkg_get_kmpkg_installed_python(KMPKG_PYTHON3)
endblock()
