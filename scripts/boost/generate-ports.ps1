...
function GeneratePortHash() {
  ...
  $hash = & $kmpkg --x-wait-for-lock hash $Archive
  # Remove prefix "Waiting to take filesystem lock on <path>/.kmpkg-root... "
  if ($hash -is [Object[]]) {
      $hash = $hash[1]
  }
  return $hash
...
function GeneratePortManifest() {
    ...
    | Out-File -Encoding UTF8 "$portsDir/$PortName/kmpkg.json"
    & $kmpkg format-manifest "$portsDir/$PortName/kmpkg.json"
}
...
if ($updateServicePorts) {
    ...
    Set-Content -LiteralPath "$portsDir/boost/portfile.cmake" `
        -Value "set(KMPKG_POLICY_EMPTY_PACKAGE enabled)`n" `
        -Encoding UTF8 `
        -NoNewline
    ...
    GeneratePortManifest `
        ...
        -Description "Internal kmpkg port used to uninstall Boost" `
        -License "MIT"
}
