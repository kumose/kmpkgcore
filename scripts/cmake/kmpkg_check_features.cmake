function(z_kmpkg_check_features_last_feature out_var features_name features_list)
    ...
    if(NOT features_length_mod_2 EQUAL 0)
        message(FATAL_ERROR "kmpkg_check_features has an incorrect number of arguments to ${features_name}")
    endif()
...
function(z_kmpkg_check_features_get_feature idx features_list out_feature_name out_feature_var)
...
function(kmpkg_check_features)
    ...
    if(NOT DEFINED arg_FEATURES AND NOT DEFINED arg_INVERTED_FEATURES)
        message(DEPRECATION
"calling `kmpkg_check_features` without the `FEATURES` keyword has been deprecated.
    Please add the `FEATURES` keyword to the call.")
        set(arg_FEATURES "${arg_UNPARSED_ARGUMENTS}")
    elseif(DEFINED arg_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR "kmpkg_check_features called with unknown arguments: ${arg_UNPARSED_ARGUMENTS}")
    endif()



    z_kmpkg_check_features_last_feature(last_feature "FEATURES" "${arg_FEATURES}")
    if(last_feature GREATER_EQUAL 0)
        foreach(feature_pair_idx RANGE "${last_feature}")
            z_kmpkg_check_features_get_feature("${feature_pair_idx}" "${arg_FEATURES}" feature_name feature_var)
    ...
    z_kmpkg_check_features_last_feature(last_inverted_feature "INVERTED_FEATURES" "${arg_INVERTED_FEATURES}")
    if(last_inverted_feature GREATER_EQUAL 0)
        foreach(feature_pair_idx RANGE "${last_inverted_feature}")
            z_kmpkg_check_features_get_feature("${feature_pair_idx}" "${arg_INVERTED_FEATURES}" feature_name feature_var)
    ...
    foreach(variable IN LISTS feature_variables)
        if(variable STREQUAL last_variable)
            message("${Z_KMPKG_BACKCOMPAT_MESSAGE_LEVEL}" "kmpkg_check_features passed the same feature variable multiple times: '${variable}'")
        endif()
