...
set(Z_KMPKG_ACQUIRE_MSYS_MIRRORS
    # Alternative primary
...
function(z_kmpkg_acquire_msys_download_package out_archive)
    cmake_parse_arguments(PARSE_ARGV 1 "arg" "" "URL;SHA512;FILENAME" "")
    if(DEFINED arg_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR "internal error: z_kmpkg_acquire_msys_download_package passed extra args: ${arg_UNPARSED_ARGUMENTS}")
    endif()

    string(REPLACE "https://repo.msys2.org/" "https://mirror.msys2.org/" all_urls "${arg_URL}")
    foreach(mirror IN LISTS Z_KMPKG_ACQUIRE_MSYS_MIRRORS)
        string(REPLACE "https://mirror.msys2.org/" "${mirror}" mirror_url "${arg_URL}")
        list(APPEND all_urls "${mirror_url}")
    endforeach()

    kmpkg_download_distfile(msys_archive
        URLS ${all_urls}
        SHA512 "${arg_SHA512}"
...
#   - Z_KMPKG_MSYS_PACKAGES_AVAILABLE
#   - Z_KMPKG_MSYS_${arg_NAME}_URL
#   - Z_KMPKG_MSYS_${arg_NAME}_SHA512
#   - Z_KMPKG_MSYS_${arg_NAME}_FILENAME
#   - Z_KMPKG_MSYS_${arg_NAME}_DEPS
#   - Z_KMPKG_MSYS_${arg_NAME}_PATCHES
#   - Z_KMPKG_MSYS_${arg_NAME}_DIRECT
#   - Z_KMPKG_MSYS_${arg_NAME}_PROVIDES
#   - Z_KMPKG_MSYS_${alias}_PROVIDED_BY
function(z_kmpkg_acquire_msys_declare_package)
    cmake_parse_arguments(PARSE_ARGV 0 arg "DIRECT" "NAME;URL;SHA512" "DEPS;PATCHES;PROVIDES")

    if(DEFINED arg_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR "internal error: z_kmpkg_acquire_msys_declare_package passed extra args: ${arg_UNPARSED_ARGUMENTS}")
    endif()
    foreach(required_arg IN ITEMS URL SHA512)
        if(NOT DEFINED arg_${required_arg})
            message(FATAL_ERROR "internal error: z_kmpkg_acquire_msys_declare_package requires argument: ${required_arg}")
        endif()
    endforeach()

    if(arg_DIRECT)
        if(NOT arg_NAME)
            message(FATAL_ERROR "internal error: z_kmpkg_acquire_msys_declare_package requires argument: NAME")
        endif()
        get_filename_component(filename "${arg_URL}" NAME)
    else()
        if(NOT arg_URL MATCHES [[^https://mirror\.msys2\.org/.*/(([^/]*)-[^-/]+-[^-/]+-[^-/]+\.pkg\.tar\.(xz|zst))$]])
            message(FATAL_ERROR "internal error: regex does not match supplied URL to kmpkg_acquire_msys: ${arg_URL}")
        endif()
        set(filename "msys2-${CMAKE_MATCH_1}")
        if(NOT DEFINED arg_NAME)
            set(arg_NAME "${CMAKE_MATCH_2}")
        endif()
        if(Z_KMPKG_MSYS_${arg_NAME}_DIRECT)
            return()
        endif()
        if(arg_NAME IN_LIST Z_KMPKG_MSYS_PACKAGES_AVAILABLE)
            message(FATAL_ERROR "Redeclaration of package '${arg_NAME}'")
        endif()
    endif()

    list(APPEND Z_KMPKG_MSYS_PACKAGES_AVAILABLE "${arg_NAME}")
    set(Z_KMPKG_MSYS_PACKAGES_AVAILABLE "${Z_KMPKG_MSYS_PACKAGES_AVAILABLE}" CACHE INTERNAL "")
    set(Z_KMPKG_MSYS_${arg_NAME}_URL "${arg_URL}" CACHE INTERNAL "")
    set(Z_KMPKG_MSYS_${arg_NAME}_SHA512 "${arg_SHA512}" CACHE INTERNAL "")
    set(Z_KMPKG_MSYS_${arg_NAME}_FILENAME "${filename}" CACHE INTERNAL "")
    set(Z_KMPKG_MSYS_${arg_NAME}_DEPS "${arg_DEPS}" CACHE INTERNAL "")
    set(Z_KMPKG_MSYS_${arg_NAME}_PATCHES "${arg_PATCHES}" CACHE INTERNAL "")
    set(Z_KMPKG_MSYS_${arg_NAME}_DIRECT "${arg_DIRECT}" CACHE INTERNAL "")
    set(Z_KMPKG_MSYS_${arg_NAME}_PROVIDES "${arg_PROVIDES}" CACHE INTERNAL "")
    foreach(name IN LISTS arg_PROVIDES)
        set(Z_KMPKG_MSYS_${name}_PROVIDED_BY "${arg_NAME}" CACHE INTERNAL "")
    endforeach()
endfunction()

# Collects all required packages to satisfy the given input set
# Writes to the following cache variables:
#   - Z_KMPKG_MSYS_<name>_ARCHIVE
function(z_kmpkg_acquire_msys_download_packages)
    ...
    while(NOT backlog STREQUAL "")
        ...
        if(DEFINED Z_KMPKG_MSYS_${name}_PROVIDED_BY AND NOT name IN_LIST Z_KMPKG_MSYS_PACKAGES_AVAILABLE)
            set(name "${Z_KMPKG_MSYS_${name}_PROVIDED_BY}")
            if(name IN_LIST resolved)
                continue()
            endif()
        endif()
        if(NOT name IN_LIST Z_KMPKG_MSYS_PACKAGES_AVAILABLE)
            list(APPEND unknown "${name}")
            continue()
        endif()
        list(APPEND resolved "${name}")
        list(REMOVE_ITEM Z_KMPKG_MSYS_${name}_DEPS ${resolved} ${backlog})
        list(APPEND backlog ${Z_KMPKG_MSYS_${name}_DEPS})

        z_kmpkg_acquire_msys_download_package(archive
            URL "${Z_KMPKG_MSYS_${name}_URL}"
            SHA512 "${Z_KMPKG_MSYS_${name}_SHA512}"
            FILENAME "${Z_KMPKG_MSYS_${name}_FILENAME}"
        )
        set(Z_KMPKG_MSYS_${name}_ARCHIVE "${archive}" CACHE INTERNAL "")
    endwhile()
...
function(z_kmpkg_acquire_msys_collect_hashes out_hash)
    cmake_parse_arguments(PARSE_ARGV 1 "arg" "" "" "PACKAGES")
    list(SORT arg_PACKAGES)
    set(result "")
    foreach(name IN LISTS arg_PACKAGES)
        if(NOT DEFINED Z_KMPKG_MSYS_${name}_SHA512)
            message(FATAL_ERROR "SHA512 unknown for '${name}'.")
        endif()
        string(APPEND result "${Z_KMPKG_MSYS_${name}_SHA512}")
        foreach(patch IN LISTS Z_KMPKG_MSYS_${name}_PATCHES)
...
function(kmpkg_acquire_msys out_msys_root)
    ...
    if(DEFINED arg_UNPARSED_ARGUMENTS)
        message(WARNING "kmpkg_acquire_msys was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}")
    endif()

    z_kmpkg_acquire_msys_declare_all_packages()
    ...
    if(arg_Z_ALL_PACKAGES)
        set(requested "${Z_KMPKG_MSYS_PACKAGES_AVAILABLE}")
    ...
    if(DEFINED arg_DIRECT_PACKAGES AND NOT arg_DIRECT_PACKAGES STREQUAL "")
        ...
        if(direct_packages_parity EQUAL 1)
            message(FATAL_ERROR "kmpkg_acquire_msys(... DIRECT_PACKAGES ...) requires exactly pairs of URL/SHA512")
        endif()
        ...
        foreach(index RANGE "${direct_packages_last}")
            ...
            z_kmpkg_acquire_msys_declare_package(
                NAME "${pkg_name}"
                URL "${url}"
                SHA512 "${sha512}"
                DIRECT
    ...
    z_kmpkg_acquire_msys_download_packages(
        PACKAGES ${requested}
        OUT_RESOLVED resolved
        OUT_UNKNOWN unknown
    )
    if(NOT unknown STREQUAL "")
        message(FATAL_ERROR "Unknown packages were required for kmpkg_acquire_msys(${requested}): ${unknown}
This can be resolved by explicitly passing URL/SHA pairs to DIRECT_PACKAGES.")
    endif()
    set(Z_KMPKG_MSYS_PACKAGES_RESOLVED "${resolved}" CACHE INTERNAL "Export for CI")

    z_kmpkg_acquire_msys_collect_hashes(hashes PACKAGES ${resolved})
    ...
    if(NOT EXISTS "${path_to_root}")
        ...
        foreach(name IN LISTS resolved)
            file(ARCHIVE_EXTRACT
                INPUT "${Z_KMPKG_MSYS_${name}_ARCHIVE}"
                DESTINATION "${path_to_root}.tmp"
            )
            if(Z_KMPKG_MSYS_${name}_PATCHES)
                z_kmpkg_apply_patches(
                    SOURCE_PATH "${path_to_root}.tmp"
                    PATCHES ${Z_KMPKG_MSYS_${name}_PATCHES}
                )
            endif()
        endforeach()
...
set(Z_KMPKG_AUTOMAKE_CLANG_CL_PATCH "${CMAKE_CURRENT_LIST_DIR}/compile_wrapper_consider_clang-cl.patch")

macro(z_kmpkg_acquire_msys_declare_all_packages)
    set(Z_KMPKG_MSYS_PACKAGES_AVAILABLE "" CACHE INTERNAL "")

    # The following list can be updated via test port kmpkg-ci-msys2[update-all].
    ...
    z_kmpkg_acquire_msys_declare_package(
        ...
        DEPS bash perl
        PATCHES "${Z_KMPKG_AUTOMAKE_CLANG_CL_PATCH}"
    )
    z_kmpkg_acquire_msys_declare_package(
    ...
    if(X_KMPKG_USE_MSYS2_RUNTIME_3.4) # temporary option, for Windows 7.0 and 8.0, or in case of regressions
        z_kmpkg_acquire_msys_declare_package(
