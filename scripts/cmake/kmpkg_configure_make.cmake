macro(z_kmpkg_determine_autotools_host_cpu out_var)
    ...
    elseif(DEFINED ENV{PROCESSOR_ARCHITECTURE})
        ...
        set(host_arch "${KMPKG_DETECTED_CMAKE_HOST_SYSTEM_PROCESSOR}")
    endif()
    ...
    elseif(host_arch MATCHES "^(ARM|arm)$")
        ...
        message(FATAL_ERROR "Unsupported host architecture ${host_arch} in z_kmpkg_determine_autotools_host_cpu!" )
    endif()
    unset(host_arch)
endmacro()

macro(z_kmpkg_determine_autotools_target_cpu out_var)
    if(KMPKG_TARGET_ARCHITECTURE MATCHES "(x|X)64")
        set(${out_var} x86_64)
    elseif(KMPKG_TARGET_ARCHITECTURE MATCHES "(x|X)86")
        set(${out_var} i686)
    elseif(KMPKG_TARGET_ARCHITECTURE MATCHES "^(ARM|arm)64$")
        set(${out_var} aarch64)
    elseif(KMPKG_TARGET_ARCHITECTURE MATCHES "^(ARM|arm)$")
        set(${out_var} arm)
    else()
        message(FATAL_ERROR "Unsupported KMPKG_TARGET_ARCHITECTURE architecture ${KMPKG_TARGET_ARCHITECTURE} in z_kmpkg_determine_autotools_target_cpu!" )
    endif()
endmacro()

macro(z_kmpkg_set_arch_mac out_var value)
...
macro(z_kmpkg_determine_autotools_host_arch_mac out_var)
    z_kmpkg_set_arch_mac(${out_var} "${KMPKG_DETECTED_CMAKE_HOST_SYSTEM_PROCESSOR}")
endmacro()

macro(z_kmpkg_determine_autotools_target_arch_mac out_var)
    list(LENGTH KMPKG_OSX_ARCHITECTURES osx_archs_num)
    if(osx_archs_num EQUAL 0)
        z_kmpkg_set_arch_mac(${out_var} "${KMPKG_DETECTED_CMAKE_HOST_SYSTEM_PROCESSOR}")
    elseif(osx_archs_num GREATER_EQUAL 2)
        set(${out_var} "universal")
    else()
        z_kmpkg_set_arch_mac(${out_var} "${KMPKG_OSX_ARCHITECTURES}")
    endif()
    unset(osx_archs_num)
endmacro()

# Define variables used in both kmpkg_configure_make and kmpkg_build_make:
...
macro(z_kmpkg_configure_make_common_definitions)
...
# Prerequisite: KMPKG_DETECTED_CMAKE_... vars loaded
function(z_kmpkg_configure_make_process_flags var_suffix)
    # separate_arguments is needed to remove outer quotes from detected cmake variables.
    # (e.g. Android NDK has "--sysroot=...")
    separate_arguments(CFLAGS NATIVE_COMMAND "Z_VCM_WRAP ${KMPKG_DETECTED_CMAKE_C_FLAGS_${var_suffix}} Z_VCM_WRAP")
    separate_arguments(CXXFLAGS NATIVE_COMMAND "Z_VCM_WRAP ${KMPKG_DETECTED_CMAKE_CXX_FLAGS_${var_suffix}} Z_VCM_WRAP")
    separate_arguments(LDFLAGS NATIVE_COMMAND "${KMPKG_DETECTED_CMAKE_SHARED_LINKER_FLAGS_${var_suffix}}")
    separate_arguments(ARFLAGS NATIVE_COMMAND "${KMPKG_DETECTED_CMAKE_STATIC_LINKER_FLAGS_${var_suffix}}")
    foreach(var IN ITEMS CFLAGS CXXFLAGS LDFLAGS ARFLAGS)
        kmpkg_list(APPEND z_vcm_all_flags ${${var}})
    endforeach()
    set(z_vcm_all_flags "${z_vcm_all_flags}" PARENT_SCOPE)

    # Filter common CPPFLAGS out of CFLAGS and CXXFLAGS
    kmpkg_list(SET CPPFLAGS)
    kmpkg_list(SET pattern)
    foreach(arg IN LISTS CXXFLAGS)
        if(arg STREQUAL "Z_VCM_WRAP")
            continue()
        elseif(NOT pattern STREQUAL "")
            kmpkg_list(APPEND pattern "${arg}")
        elseif(arg MATCHES "^-(D|isystem).")
            kmpkg_list(SET pattern "${arg}")
        elseif(arg MATCHES "^-(D|isystem)\$")
            kmpkg_list(SET pattern "${arg}")
            continue()
        ...
        if(NOT index STREQUAL "-1")
            kmpkg_list(APPEND CPPFLAGS ${pattern})
    ...
    foreach(list IN ITEMS CFLAGS CXXFLAGS)
        kmpkg_list(REMOVE_ITEM ${list} "Z_VCM_WRAP")
    endforeach()
    ...
    if(KMPKG_DETECTED_CMAKE_C_COMPILER_FRONTEND_VARIANT STREQUAL "MSVC" OR KMPKG_DETECTED_CMAKE_C_COMPILER_ID STREQUAL "MSVC")
    ...
    # Could use a future KMPKG_DETECTED_CMAKE_LIBRARY_PATH_FLAG
    set(library_path_flag "-L")
    # Could use a future KMPKG_DETECTED_MSVC
    if(KMPKG_TARGET_IS_WINDOWS AND KMPKG_DETECTED_CMAKE_LINKER MATCHES [[link\.exe$]])
        set(library_path_flag "-LIBPATH:")
    endif()
    set(linker_flag_escape "")
    if(KMPKG_TARGET_IS_WINDOWS AND KMPKG_DETECTED_CMAKE_C_COMPILER MATCHES [[cl\.exe$]])
        # Removed by libtool
        ...
        if(KMPKG_LIBRARY_LINKAGE STREQUAL "static")
            string(STRIP "$ENV{_LINK_} ${KMPKG_DETECTED_CMAKE_STATIC_LINKER_FLAGS_${var_suffix}}" LINK_ENV)
        else()
            string(STRIP "$ENV{_LINK_} ${KMPKG_DETECTED_CMAKE_SHARED_LINKER_FLAGS_${var_suffix}}" LINK_ENV)
        endif()
    endif()
    ...
    if(EXISTS "${CURRENT_INSTALLED_DIR}${path_suffix_${var_suffix}}/lib/manual-link")
        kmpkg_list(PREPEND LDFLAGS "${linker_flag_escape}${library_path_flag}${current_installed_dir_escaped}${path_suffix_${var_suffix}}/lib/manual-link")
    endif()
    if(EXISTS "${CURRENT_INSTALLED_DIR}${path_suffix_${var_suffix}}/lib")
        kmpkg_list(PREPEND LDFLAGS "${linker_flag_escape}${library_path_flag}${current_installed_dir_escaped}${path_suffix_${var_suffix}}/lib")
    endif()
...
macro(z_kmpkg_append_to_configure_environment inoutstring var defaultval)
...
function(kmpkg_configure_make)
    ...
    if(DEFINED KMPKG_MAKE_BUILD_TRIPLET)
        set(arg_BUILD_TRIPLET ${KMPKG_MAKE_BUILD_TRIPLET}) # Triplet overwrite for crosscompiling
    endif()
    ...
    if(CMAKE_HOST_WIN32 AND KMPKG_DETECTED_CMAKE_C_COMPILER MATCHES "cl.exe") #only applies to windows (clang-)cl and lib
        if(arg_AUTOCONFIG)
    ...
    kmpkg_backup_env_variables(VARS ${cm_FLAGS})
    ...
    # Used by gcc/linux
    kmpkg_backup_env_variables(VARS C_INCLUDE_PATH CPLUS_INCLUDE_PATH LIBRARY_PATH LD_LIBRARY_PATH)

    # Used by cl
    kmpkg_backup_env_variables(VARS INCLUDE LIB LIBPATH)
    ...
    if(CMAKE_HOST_WIN32)
        ...
        kmpkg_acquire_msys(MSYS_ROOT PACKAGES ${msys_require_packages} ${arg_ADDITIONAL_MSYS_PACKAGES})
        set(base_cmd "${MSYS_ROOT}/usr/bin/bash.exe" --noprofile --norc --debug)
        kmpkg_list(SET add_to_env)
        if(arg_USE_WRAPPERS AND KMPKG_TARGET_IS_WINDOWS)
            kmpkg_list(APPEND add_to_env "${SCRIPTS}/buildsystems/make_wrapper") # Other required wrappers are also located there
            kmpkg_list(APPEND add_to_env "${MSYS_ROOT}/usr/share/automake-1.16")
        endif()
        ...
        kmpkg_list(INSERT path_list "${index}" ${add_to_env} "${MSYS_ROOT}/usr/bin")
   ...
   # Apple platforms - cross-compiling support
    if(KMPKG_TARGET_IS_APPLE)
        if (requires_autoconfig AND NOT arg_BUILD_TRIPLET OR arg_DETERMINE_BUILD_TRIPLET)
            z_kmpkg_determine_autotools_host_arch_mac(BUILD_ARCH) # machine you are building on => --build=
            z_kmpkg_determine_autotools_target_arch_mac(TARGET_ARCH)
            # --build: the machine you are building on
            # --host: the machine you are building for
            # --target: the machine that CC will produce binaries for
            # https://stackoverflow.com/questions/21990021/how-to-determine-host-value-for-configure-when-using-cross-compiler
            # Only for ports using autotools so we can assume that they follow the common conventions for build/target/host
            if(NOT "${TARGET_ARCH}" STREQUAL "${BUILD_ARCH}" OR NOT KMPKG_TARGET_IS_OSX) # we don't need to specify the additional flags if we build natively.
    ...
    if(KMPKG_TARGET_IS_LINUX)
        if (requires_autoconfig AND NOT arg_BUILD_TRIPLET OR arg_DETERMINE_BUILD_TRIPLET)
            ...
            if(KMPKG_DETECTED_CMAKE_C_COMPILER MATCHES "([^\/]*)-gcc$" AND CMAKE_MATCH_1)
    ...
    if (KMPKG_TARGET_IS_WINDOWS)
        if (arg_DETERMINE_BUILD_TRIPLET OR NOT arg_BUILD_TRIPLET)
            z_kmpkg_determine_autotools_host_cpu(BUILD_ARCH) # KMPKG_HOST => machine you are building on => --build=
            z_kmpkg_determine_autotools_target_cpu(TARGET_ARCH)
            # --build: the machine you are building on
            # --host: the machine you are building for
            # --target: the machine that CC will produce binaries for
            # https://stackoverflow.com/questions/21990021/how-to-determine-host-value-for-configure-when-using-cross-compiler
            ...
            if(KMPKG_TARGET_IS_UWP AND NOT arg_BUILD_TRIPLET MATCHES "--host")
        ...
        set(progs KMPKG_DETECTED_CMAKE_C_COMPILER KMPKG_DETECTED_CMAKE_CXX_COMPILER KMPKG_DETECTED_CMAKE_AR
                  KMPKG_DETECTED_CMAKE_LINKER KMPKG_DETECTED_CMAKE_RANLIB KMPKG_DETECTED_CMAKE_OBJDUMP
                  KMPKG_DETECTED_CMAKE_STRIP KMPKG_DETECTED_CMAKE_NM KMPKG_DETECTED_CMAKE_DLLTOOL KMPKG_DETECTED_CMAKE_RC_COMPILER)
        foreach(prog IN LISTS progs)
            ...
            if(filepath MATCHES " ")
                ...
                if(NOT z_vcm_prog_found STREQUAL filepath)
                    ...
                    kmpkg_add_to_path(PREPEND "${dir}")
                endif()
            endif()
        endforeach()
        if (arg_USE_WRAPPERS)
            z_kmpkg_append_to_configure_environment(configure_env CPP "compile ${KMPKG_DETECTED_CMAKE_C_COMPILER} -E")

            z_kmpkg_append_to_configure_environment(configure_env CC "compile ${KMPKG_DETECTED_CMAKE_C_COMPILER}")
            if(NOT arg_BUILD_TRIPLET MATCHES "--host")
                z_kmpkg_append_to_configure_environment(configure_env CC_FOR_BUILD "compile ${KMPKG_DETECTED_CMAKE_C_COMPILER}")
                z_kmpkg_append_to_configure_environment(configure_env CPP_FOR_BUILD "compile ${KMPKG_DETECTED_CMAKE_C_COMPILER} -E")
                z_kmpkg_append_to_configure_environment(configure_env CXX_FOR_BUILD "compile ${KMPKG_DETECTED_CMAKE_CXX_COMPILER}")
            else()
                # Silly trick to make configure accept CC_FOR_BUILD but in reallity CC_FOR_BUILD is deactivated.
                z_kmpkg_append_to_configure_environment(configure_env CC_FOR_BUILD "touch a.out | touch conftest${KMPKG_HOST_EXECUTABLE_SUFFIX} | true")
                z_kmpkg_append_to_configure_environment(configure_env CPP_FOR_BUILD "touch a.out | touch conftest${KMPKG_HOST_EXECUTABLE_SUFFIX} | true")
                z_kmpkg_append_to_configure_environment(configure_env CXX_FOR_BUILD "touch a.out | touch conftest${KMPKG_HOST_EXECUTABLE_SUFFIX} | true")
            endif()
            z_kmpkg_append_to_configure_environment(configure_env CXX "compile ${KMPKG_DETECTED_CMAKE_CXX_COMPILER}")
            z_kmpkg_append_to_configure_environment(configure_env RC "windres-rc ${KMPKG_DETECTED_CMAKE_RC_COMPILER}")
            z_kmpkg_append_to_configure_environment(configure_env WINDRES "windres-rc ${KMPKG_DETECTED_CMAKE_RC_COMPILER}")
            if(KMPKG_DETECTED_CMAKE_AR)
                z_kmpkg_append_to_configure_environment(configure_env AR "ar-lib ${KMPKG_DETECTED_CMAKE_AR}")
            else()
                z_kmpkg_append_to_configure_environment(configure_env AR "ar-lib lib.exe -verbose")
            endif()
        else()
            z_kmpkg_append_to_configure_environment(configure_env CPP "${KMPKG_DETECTED_CMAKE_C_COMPILER} -E")
            z_kmpkg_append_to_configure_environment(configure_env CC "${KMPKG_DETECTED_CMAKE_C_COMPILER}")
            if(NOT arg_BUILD_TRIPLET MATCHES "--host")
                z_kmpkg_append_to_configure_environment(configure_env CC_FOR_BUILD "${KMPKG_DETECTED_CMAKE_C_COMPILER}")
                z_kmpkg_append_to_configure_environment(configure_env CPP_FOR_BUILD "${KMPKG_DETECTED_CMAKE_C_COMPILER} -E")
                z_kmpkg_append_to_configure_environment(configure_env CXX_FOR_BUILD "${KMPKG_DETECTED_CMAKE_CXX_COMPILER}")
            else()
                z_kmpkg_append_to_configure_environment(configure_env CC_FOR_BUILD "touch a.out | touch conftest${KMPKG_HOST_EXECUTABLE_SUFFIX} | true")
                z_kmpkg_append_to_configure_environment(configure_env CPP_FOR_BUILD "touch a.out | touch conftest${KMPKG_HOST_EXECUTABLE_SUFFIX} | true")
                z_kmpkg_append_to_configure_environment(configure_env CXX_FOR_BUILD "touch a.out | touch conftest${KMPKG_HOST_EXECUTABLE_SUFFIX} | true")
            endif()
            z_kmpkg_append_to_configure_environment(configure_env CXX "${KMPKG_DETECTED_CMAKE_CXX_COMPILER}")
            z_kmpkg_append_to_configure_environment(configure_env RC "${KMPKG_DETECTED_CMAKE_RC_COMPILER}")
            z_kmpkg_append_to_configure_environment(configure_env WINDRES "${KMPKG_DETECTED_CMAKE_RC_COMPILER}")
            if(KMPKG_DETECTED_CMAKE_AR)
                z_kmpkg_append_to_configure_environment(configure_env AR "${KMPKG_DETECTED_CMAKE_AR}")
            else()
                z_kmpkg_append_to_configure_environment(configure_env AR "lib.exe -verbose")
            endif()
        endif()
        z_kmpkg_append_to_configure_environment(configure_env LD "${KMPKG_DETECTED_CMAKE_LINKER} -verbose")
        if(KMPKG_DETECTED_CMAKE_RANLIB)
            z_kmpkg_append_to_configure_environment(configure_env RANLIB "${KMPKG_DETECTED_CMAKE_RANLIB}") # Trick to ignore the RANLIB call
        else()
            z_kmpkg_append_to_configure_environment(configure_env RANLIB ":")
        endif()
        if(KMPKG_DETECTED_CMAKE_OBJDUMP) #Objdump is required to make shared libraries. Otherwise define lt_cv_deplibs_check_method=pass_all
            z_kmpkg_append_to_configure_environment(configure_env OBJDUMP "${KMPKG_DETECTED_CMAKE_OBJDUMP}") # Trick to ignore the RANLIB call
        endif()
        if(KMPKG_DETECTED_CMAKE_STRIP) # If required set the ENV variable STRIP in the portfile correctly
            z_kmpkg_append_to_configure_environment(configure_env STRIP "${KMPKG_DETECTED_CMAKE_STRIP}")
        else()
            z_kmpkg_append_to_configure_environment(configure_env STRIP ":")
            list(APPEND arg_OPTIONS ac_cv_prog_ac_ct_STRIP=:)
        endif()
        if(KMPKG_DETECTED_CMAKE_NM) # If required set the ENV variable NM in the portfile correctly
            z_kmpkg_append_to_configure_environment(configure_env NM "${KMPKG_DETECTED_CMAKE_NM}")
        else()
            ...
            z_kmpkg_append_to_configure_environment(configure_env NM "dumpbin.exe -symbols -headers")
        endif()
        if(KMPKG_DETECTED_CMAKE_DLLTOOL) # If required set the ENV variable DLLTOOL in the portfile correctly
            z_kmpkg_append_to_configure_environment(configure_env DLLTOOL "${KMPKG_DETECTED_CMAKE_DLLTOOL}")
        else()
            z_kmpkg_append_to_configure_environment(configure_env DLLTOOL "link.exe -verbose -dll")
        endif()
        z_kmpkg_append_to_configure_environment(configure_env CCAS ":")   # If required set the ENV variable CCAS in the portfile correctly
        z_kmpkg_append_to_configure_environment(configure_env AS ":")   # If required set the ENV variable AS in the portfile correctly

        foreach(_env IN LISTS arg_CONFIGURE_ENVIRONMENT_VARIABLES)
            z_kmpkg_append_to_configure_environment(configure_env ${_env} "${${_env}}")
        endforeach()
        ...
        if(KMPKG_TARGET_ARCHITECTURE MATCHES "^[Aa][Rr][Mm]64$")
        ...
        elseif(KMPKG_TARGET_ARCHITECTURE MATCHES "^[Aa][Rr][Mm]$")
        ...
        kmpkg_list(SET z_vcm_all_tools)
        function(z_kmpkg_make_set_env envvar cmakevar)
            if(NOT KMPKG_DETECTED_CMAKE_${cmakevar})
              return()
            endif()
            set(prog "${KMPKG_DETECTED_CMAKE_${cmakevar}}")
            if(NOT DEFINED ENV{${envvar}} AND NOT prog STREQUAL "")
                kmpkg_list(APPEND z_vcm_all_tools "${prog}")
                if(ARGN)
        ...
        z_kmpkg_make_set_env(CC C_COMPILER)
        if(NOT arg_BUILD_TRIPLET MATCHES "--host")
            z_kmpkg_make_set_env(CC_FOR_BUILD C_COMPILER)
            z_kmpkg_make_set_env(CPP_FOR_BUILD C_COMPILER "-E")
            z_kmpkg_make_set_env(CXX_FOR_BUILD C_COMPILER)
        else()
            set(ENV{CC_FOR_BUILD} "touch a.out | touch conftest${KMPKG_HOST_EXECUTABLE_SUFFIX} | true")
            set(ENV{CPP_FOR_BUILD} "touch a.out | touch conftest${KMPKG_HOST_EXECUTABLE_SUFFIX} | true")
            set(ENV{CXX_FOR_BUILD} "touch a.out | touch conftest${KMPKG_HOST_EXECUTABLE_SUFFIX} | true")
        endif()
        z_kmpkg_make_set_env(CXX CXX_COMPILER)
        z_kmpkg_make_set_env(NM NM)
        z_kmpkg_make_set_env(RC RC)
        z_kmpkg_make_set_env(WINDRES RC)
        z_kmpkg_make_set_env(DLLTOOL DLLTOOL)
        z_kmpkg_make_set_env(STRIP STRIP)
        z_kmpkg_make_set_env(OBJDUMP OBJDUMP)
        z_kmpkg_make_set_env(RANLIB RANLIB)
        z_kmpkg_make_set_env(AR AR)
        z_kmpkg_make_set_env(LD LINKER)
        unset(z_kmpkg_make_set_env)
    endif()
    ...
    # Set configure paths
    kmpkg_list(APPEND arg_OPTIONS_RELEASE "--prefix=${current_installed_dir_msys}")
    kmpkg_list(APPEND arg_OPTIONS_DEBUG "--prefix=${current_installed_dir_msys}${path_suffix_DEBUG}")
    if(NOT arg_NO_ADDITIONAL_PATHS)
        # ${prefix} has an extra backslash to prevent early expansion when calling `bash -c configure "..."`.
        kmpkg_list(APPEND arg_OPTIONS_RELEASE
        ...
        kmpkg_list(APPEND arg_OPTIONS_DEBUG
                            # Important: These should all be relative to prefix!
    ...
    if(DEFINED KMPKG_CONFIGURE_MAKE_OPTIONS)
        list(APPEND arg_OPTIONS ${KMPKG_CONFIGURE_MAKE_OPTIONS})
    endif()
    if(DEFINED KMPKG_CONFIGURE_MAKE_OPTIONS_RELEASE)
        list(APPEND arg_OPTIONS_RELEASE ${KMPKG_CONFIGURE_MAKE_OPTIONS_RELEASE})
    endif()
    if(DEFINED KMPKG_CONFIGURE_MAKE_OPTIONS_DEBUG)
        list(APPEND arg_OPTIONS_DEBUG ${KMPKG_CONFIGURE_MAKE_OPTIONS_DEBUG})
    endif()
    ...
    # Flags should be set in the toolchain instead (Setting this up correctly requires a function named kmpkg_determined_cmake_compiler_flags which can also be used to setup CC and CXX etc.)
    if(KMPKG_TARGET_IS_WINDOWS)
        kmpkg_backup_env_variables(VARS _CL_ _LINK_)
        # TODO: Should be CPP flags instead -> rewrite when kmpkg_determined_cmake_compiler_flags defined
        if(KMPKG_TARGET_IS_UWP)
            ...
            string(REPLACE "${_replacement}" "" KMPKG_DETECTED_CMAKE_CXX_FLAGS_DEBUG "${KMPKG_DETECTED_CMAKE_CXX_FLAGS_DEBUG}")
            string(REPLACE "${_replacement}" "" KMPKG_DETECTED_CMAKE_C_FLAGS_DEBUG "${KMPKG_DETECTED_CMAKE_C_FLAGS_DEBUG}")
            string(REPLACE "${_replacement}" "" KMPKG_DETECTED_CMAKE_CXX_FLAGS_RELEASE "${KMPKG_DETECTED_CMAKE_CXX_FLAGS_RELEASE}")
            string(REPLACE "${_replacement}" "" KMPKG_DETECTED_CMAKE_C_FLAGS_RELEASE "${KMPKG_DETECTED_CMAKE_C_FLAGS_RELEASE}")
            ...
            set(ENV{_LINK_} "$ENV{_LINK_} ${KMPKG_DETECTED_CMAKE_C_STANDARD_LIBRARIES} ${KMPKG_DETECTED_CMAKE_CXX_STANDARD_LIBRARIES}")
        endif()
    endif()
    ...
    separate_arguments(c_libs_list NATIVE_COMMAND "${KMPKG_DETECTED_CMAKE_C_STANDARD_LIBRARIES}")
    separate_arguments(cxx_libs_list NATIVE_COMMAND "${KMPKG_DETECTED_CMAKE_CXX_STANDARD_LIBRARIES}")
    ...
    set(x_kmpkg_transform_libs ON)
    if(KMPKG_TARGET_IS_UWP)
        set(x_kmpkg_transform_libs OFF)
    ...
    if(NOT DEFINED KMPKG_BUILD_TYPE OR KMPKG_BUILD_TYPE STREQUAL "debug" AND NOT arg_NO_DEBUG)
        list(APPEND all_buildtypes DEBUG)
        z_kmpkg_configure_make_process_flags(DEBUG)
    endif()
    if(NOT DEFINED KMPKG_BUILD_TYPE OR KMPKG_BUILD_TYPE STREQUAL "release")
        list(APPEND all_buildtypes RELEASE)
        z_kmpkg_configure_make_process_flags(RELEASE)
    endif()
    ...
    foreach(var IN ITEMS arg_OPTIONS arg_OPTIONS_RELEASE arg_OPTIONS_DEBUG)
        ...
        foreach(element IN LISTS "${var}")
            ...
            kmpkg_list(APPEND tmp "\"${element}\"")
        endforeach()
        kmpkg_list(JOIN tmp " " "${var}")
    endforeach()
    ...
    foreach(current_buildtype IN LISTS all_buildtypes)
        ...
        z_kmpkg_setup_pkgconfig_path(CONFIG "${current_buildtype}")
        ...
        set(ENV{RCFLAGS} "${KMPKG_DETECTED_CMAKE_RC_FLAGS_${current_buildtype}}")
        set(ENV{LDFLAGS} "${LDFLAGS_${current_buildtype}}")
        set(ENV{LDFLAGS_FOR_BUILD} "${LDFLAGS_${current_buildtype}}")
        if(ARFLAGS_${current_buildtype} AND NOT (arg_USE_WRAPPERS AND KMPKG_TARGET_IS_WINDOWS))
        ...
        foreach(lib_env_var IN LISTS lib_env_vars)
            if(EXISTS "${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/lib")
                kmpkg_host_path_list(PREPEND ENV{${lib_env_var}} "${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/lib")
            endif()
            if(EXISTS "${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/lib/manual-link")
                kmpkg_host_path_list(PREPEND ENV{${lib_env_var}} "${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/lib/manual-link")
            endif()
        endforeach()
        unset(lib_env_vars)
        ...
        if(arg_ADD_BIN_TO_PATH)
            ...
            kmpkg_add_to_path("${CURRENT_INSTALLED_DIR}${path_suffix_${current_buildtype}}/bin")
        endif()
        ...
        if (NOT arg_SKIP_CONFIGURE)
            ...
            if(KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW AND KMPKG_LIBRARY_LINKAGE STREQUAL dynamic)
    ...
    # Export matching make program for kmpkg_build_make (cache variable)
    if(CMAKE_HOST_WIN32 AND MSYS_ROOT)
        find_program(Z_KMPKG_MAKE make PATHS "${MSYS_ROOT}/usr/bin" NO_DEFAULT_PATH REQUIRED)
    elseif(KMPKG_HOST_IS_BSD)
        find_program(Z_KMPKG_MAKE gmake REQUIRED)
    elseif(KMPKG_HOST_IS_SOLARIS)
        find_program(Z_KMPKG_MAKE NAMES gmake make REQUIRED)
    else()
        find_program(Z_KMPKG_MAKE make REQUIRED)
    endif()

    # Restore environment
    kmpkg_restore_env_variables(VARS ${cm_FLAGS} LIB LIBPATH LIBRARY_PATH LD_LIBRARY_PATH)

    set(_KMPKG_PROJECT_SOURCE_PATH ${arg_SOURCE_PATH} PARENT_SCOPE)
    set(_KMPKG_PROJECT_SUBPATH ${arg_PROJECT_SUBPATH} PARENT_SCOPE)
    set(_KMPKG_MAKE_NO_DEBUG ${arg_NO_DEBUG} PARENT_SCOPE)
endfunction()
