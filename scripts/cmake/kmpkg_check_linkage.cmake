function(kmpkg_check_linkage)
    ...
    if(arg_ONLY_STATIC_LIBRARY AND "${KMPKG_LIBRARY_LINKAGE}" STREQUAL "dynamic")
        message(STATUS "Note: ${PORT} only supports static library linkage. Building static library.")
        set(KMPKG_LIBRARY_LINKAGE static PARENT_SCOPE)
    elseif(arg_ONLY_DYNAMIC_LIBRARY AND "${KMPKG_LIBRARY_LINKAGE}" STREQUAL "static")
...
kmpkg fails rather than changing KMPKG_LIBRARY_LINKAGE to dynamic.\

Consider choosing a triplet that sets KMPKG_CRT_LINKAGE to dynamic. For more information, \
explicitly requesting this configuration in a custom triplet, please see \
https://learn.microsoft.com/kmpkg/maintainers/functions/kmpkg_check_linkage?WT.mc_id=kmpkg_inproduct_cli#notes \

If you can edit the port calling kmpkg_check_linkage that emits this message, consider adding \
!(static & staticcrt) to the \"supports\" expression so that this combination can fail early.")
    ...
    if(arg_ONLY_DYNAMIC_CRT AND "${KMPKG_CRT_LINKAGE}" STREQUAL "static")
        message(FATAL_ERROR "${PORT} only supports dynamic crt linkage")
    elseif(arg_ONLY_STATIC_CRT AND "${KMPKG_CRT_LINKAGE}" STREQUAL "dynamic")
