function(kmpkg_execute_required_process)
    ...
    if (KMPKG_DOWNLOAD_MODE AND NOT arg_ALLOW_IN_DOWNLOAD_MODE)
        message(FATAL_ERROR
[[
...
Halting portfile execution.
    ...
    kmpkg_list(SET logfiles)
    kmpkg_list(SET logfile_copies)
    set(expect_alias FALSE)
    foreach(item IN LISTS arg_SAVE_LOG_FILES)
        if(expect_alias)
            kmpkg_list(POP_BACK logfile_copies)
            kmpkg_list(APPEND logfile_copies "${CURRENT_BUILDTREES_DIR}/${arg_LOGNAME}-${item}")
            set(expect_alias FALSE)
    ...
    if(output_and_error_same)
        z_kmpkg_forward_output_variable(arg_OUTPUT_VARIABLE out_err_var)
        # arg_ERROR_VARIABLE = arg_OUTPUT_VARIABLE, so no need to set it again
    else()
        z_kmpkg_forward_output_variable(arg_OUTPUT_VARIABLE out_var)
        z_kmpkg_forward_output_variable(arg_ERROR_VARIABLE err_var)
    endif()
endfunction()
