list(APPEND Z_KMPKG_ACQUIRE_MSYS_DECLARE_PACKAGE_COMMANDS "z_kmpkg_find_fortran_declare_msys_packages")

function(kmpkg_find_fortran out_var)
    if("${ARGC}" GREATER "1")
        message(WARNING "${CMAKE_CURRENT_FUNCTION} was passed extra args: ${ARGN}")
    endif()

    kmpkg_list(SET additional_cmake_args)

    set(CMAKE_BINARY_DIR "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}")
    set(CMAKE_CURRENT_BINARY_DIR "${CMAKE_BINARY_DIR}")
    set(CMAKE_PLATFORM_INFO_DIR "${CMAKE_BINARY_DIR}/Platform")
    macro(z_kmpkg_warn_ambiguous_system_variables)
    # CMakeDetermineFortranCompiler is for project mode.
    endmacro()
    include(CMakeDetermineFortranCompiler)

    if(NOT CMAKE_Fortran_COMPILER AND "${KMPKG_CHAINLOAD_TOOLCHAIN_FILE}" STREQUAL "")
        # If a user uses their own KMPKG_CHAINLOAD_TOOLCHAIN_FILE, they _must_ figure out fortran on their own.
        if(CMAKE_HOST_WIN32)
            message(STATUS "No Fortran compiler found on the PATH. Using MinGW gfortran!")
            # If no Fortran compiler is on the path we switch to use gfortan from MinGW within kmpkg
            if("${KMPKG_TARGET_ARCHITECTURE}" STREQUAL "x86")
                set(mingw_path mingw32)
                set(machine_flag -m32)
                kmpkg_acquire_msys(msys_root
                    NO_DEFAULT_PACKAGES
                    Z_DECLARE_EXTRA_PACKAGES_COMMAND "z_kmpkg_find_fortran_msys_declare_packages"
            ...
            elseif("${KMPKG_TARGET_ARCHITECTURE}" STREQUAL "x64")
                ...
                message(FATAL_ERROR "Unknown architecture '${KMPKG_TARGET_ARCHITECTURE}' for MinGW Fortran build!")
            endif()

            set(mingw_bin "${msys_root}/${mingw_path}/bin")
            kmpkg_add_to_path(PREPEND "${mingw_bin}")
            kmpkg_list(APPEND additional_cmake_args
                -DCMAKE_GNUtoMS=ON
                "-DCMAKE_Fortran_COMPILER=${mingw_bin}/gfortran.exe"
                "-DCMAKE_C_COMPILER=${mingw_bin}/gcc.exe"
                "-DCMAKE_Fortran_FLAGS_INIT:STRING= -mabi=ms ${machine_flag} ${KMPKG_Fortran_FLAGS}")

            # This is for private use by kmpkg-gfortran
            set(kmpkg_find_fortran_MSYS_ROOT "${msys_root}" PARENT_SCOPE)
            set(KMPKG_USE_INTERNAL_Fortran TRUE PARENT_SCOPE)
            set(KMPKG_POLICY_SKIP_DUMPBIN_CHECKS enabled PARENT_SCOPE)
            set(KMPKG_CHAINLOAD_TOOLCHAIN_FILE "${SCRIPTS}/toolchains/mingw.cmake" PARENT_SCOPE) # Switching to MinGW toolchain for Fortran
            if(KMPKG_CRT_LINKAGE STREQUAL "static")
                set(KMPKG_CRT_LINKAGE dynamic PARENT_SCOPE)
                message(STATUS "KMPKG_CRT_LINKAGE linkage for ${PORT} using kmpkg's internal gfortran cannot be static due to linking against MinGW libraries. Forcing dynamic CRT linkage")
            endif()
            if(KMPKG_LIBRARY_LINKAGE STREQUAL "static")
                set(KMPKG_LIBRARY_LINKAGE dynamic PARENT_SCOPE)
                message(STATUS "KMPKG_LIBRARY_LINKAGE linkage for ${PORT} using kmpkg's internal gfortran cannot be static due to linking against MinGW libraries. Forcing dynamic library linkage")
            endif()
        else()
...
macro(z_kmpkg_find_fortran_msys_declare_packages)
