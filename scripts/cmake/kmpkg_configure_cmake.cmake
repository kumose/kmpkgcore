function(z_kmpkg_configure_cmake_both_or_neither_set var1 var2)
...
function(z_kmpkg_configure_cmake_build_cmakecache out_var whereat build_type)
...
function(z_kmpkg_get_visual_studio_generator)
    ...
    if(DEFINED ENV{VisualStudioVersion})
        if("$ENV{VisualStudioVersion}" VERSION_LESS_EQUAL  "12.99" AND
           ...
           NOT "${KMPKG_TARGET_ARCHITECTURE}" STREQUAL "arm64")
...
function(z_kmpkg_select_default_kmpkg_chainload_toolchain)
    set(KMPKG_CHAINLOAD_TOOLCHAIN_FILE "${Z_KMPKG_CHAINLOAD_TOOLCHAIN_FILE}" PARENT_SCOPE)
endfunction()


function(kmpkg_configure_cmake)
    ...
    if(NOT arg_Z_GET_CMAKE_VARS_USAGE AND Z_KMPKG_CMAKE_CONFIGURE_GUARD)
        message(FATAL_ERROR "The ${PORT} port already depends on kmpkg-cmake; using both kmpkg-cmake and kmpkg_configure_cmake in the same port is unsupported.")
    endif()
    ...
    kmpkg_list(SET manually_specified_variables)
    ...
    if(NOT arg_PREFER_NINJA AND KMPKG_TARGET_IS_WINDOWS AND NOT KMPKG_TARGET_IS_MINGW)
    ...
    elseif(NOT ninja_can_be_used)
        set(generator "")
        z_kmpkg_get_visual_studio_generator(OUT_GENERATOR generator OUT_ARCH generator_arch)
    ...
    platform toolset: ${KMPKG_PLATFORM_TOOLSET}
    architecture    : ${KMPKG_TARGET_ARCHITECTURE}")
        endif()
    ...
    if("${generator}" STREQUAL "Ninja" AND NOT DEFINED ENV{KMPKG_FORCE_SYSTEM_BINARIES})
    ...
    z_kmpkg_configure_cmake_both_or_neither_set(KMPKG_CXX_FLAGS_DEBUG KMPKG_C_FLAGS_DEBUG)
    z_kmpkg_configure_cmake_both_or_neither_set(KMPKG_CXX_FLAGS_RELEASE KMPKG_C_FLAGS_RELEASE)
    z_kmpkg_configure_cmake_both_or_neither_set(KMPKG_CXX_FLAGS KMPKG_C_FLAGS)
    ...
    if(ninja_host AND CMAKE_HOST_WIN32 AND NOT arg_DISABLE_PARALLEL_CONFIGURE)
        ...
        file(MAKE_DIRECTORY "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/kmpkg-parallel-configure")
        file(WRITE
            "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/kmpkg-parallel-configure/build.ninja"
            "${ninja_configure_contents}")

        message(STATUS "${configuring_message}")
        kmpkg_execute_required_process(
            COMMAND "${NINJA}" -v
            WORKING_DIRECTORY "${CURRENT_BUILDTREES_DIR}/${TARGET_TRIPLET}-rel/kmpkg-parallel-configure"
            LOGNAME "${arg_LOGNAME}"
            SAVE_LOG_FILES
    ...
    kmpkg_list(SET all_unused_variables)
    ...
    ${all_unused_variables}
Please recheck them and remove the unnecessary options from the `kmpkg_configure_cmake` call.
If these options should still be passed for whatever reason, please use the `MAYBE_UNUSED_VARIABLES` argument.")
    ...
    if(NOT arg_Z_GET_CMAKE_VARS_USAGE)
        set(Z_KMPKG_CMAKE_GENERATOR "${generator}" PARENT_SCOPE)
    endif()
endfunction()
