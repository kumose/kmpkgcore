function(z_kmpkg_install_gn_get_target_type out_var)
...
function(z_kmpkg_install_gn_get_desc out_var)
...
function(z_kmpkg_install_gn_install)
    ...
    foreach(target IN LISTS arg_TARGETS)
        ...
        z_kmpkg_install_gn_get_desc(outputs
            SOURCE_PATH "${arg_SOURCE_PATH}"
            BUILD_DIR "${arg_BUILD_DIR}"
            TARGET "//${target}"
            WHAT_TO_DISPLAY outputs
        )
        z_kmpkg_install_gn_get_target_type(target_type
            SOURCE_PATH "${arg_SOURCE_PATH}"
            BUILD_DIR "${arg_BUILD_DIR}"
            TARGET "//${target}"
        )
...
function(kmpkg_install_gn)
    if(Z_KMPKG_GN_INSTALL_GUARD)
        message(FATAL_ERROR "The ${PORT} port already depends on kmpkg-gn; using both kmpkg-gn and kmpkg_install_gn in the same port is unsupported.")
    else()
        message("${Z_KMPKG_BACKCOMPAT_MESSAGE_LEVEL}" "This function 'kmpkg_install_gn' is obsolete. Use 'kmpkg_gn_install' in port 'kmpkg-gn'.")
    endif()

    cmake_parse_arguments(PARSE_ARGV 0 arg "" "SOURCE_PATH" "TARGETS")
    if(DEFINED arg_UNPARSED_ARGUMENTS)
        message(WARNING "kmpkg_install_gn was passed extra arguments: ${arg_UNPARSED_ARGUMENTS}")
    endif()
