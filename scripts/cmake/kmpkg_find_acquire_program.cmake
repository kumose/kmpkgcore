function(z_kmpkg_find_acquire_program_version_check out_var)
    ...
    kmpkg_execute_in_download_mode(
        COMMAND ${arg_COMMAND}
        WORKING_DIRECTORY "${KMPKG_ROOT_DIR}"
...
function(z_kmpkg_find_acquire_program_find_external program)
    cmake_parse_arguments(PARSE_ARGV 1 arg
        "EXACT_VERSION_MATCH"
    ...
    if("${arg_INTERPRETER}" STREQUAL "")
        ...
        if(SCRIPT_${arg_PROGRAM_NAME})
            kmpkg_list(SET program_tmp ${${interpreter}} ${SCRIPT_${arg_PROGRAM_NAME}})
    ...
    elseif(${program}) # only do a version check if ${program} has a value
        z_kmpkg_find_acquire_program_version_check(version_is_good
...
function(z_kmpkg_find_acquire_program_find_internal program)
    cmake_parse_arguments(PARSE_ARGV 1 arg
        ""
        "INTERPRETER"
        "NAMES;PATHS"
    ...
    if("${arg_INTERPRETER}" STREQUAL "")
        ...
        kmpkg_find_acquire_program("${arg_INTERPRETER}")
...
function(kmpkg_find_acquire_program program)
    if(${program})
        return()
    endif()
    ...
    set(paths_to_search "")
    set(version_command "")
    kmpkg_list(SET sourceforge_args)
    set(brew_package_name "")
    set(apt_package_name "")

    set(program_information "${CMAKE_CURRENT_FUNCTION_LIST_DIR}/kmpkg_find_acquire_program(${program}).cmake")
    ...
    z_kmpkg_find_acquire_program_find_internal("${program}"
        INTERPRETER "${interpreter}"
        PATHS ${paths_to_search}
        NAMES ${search_names}
    )
    if(NOT ${program})
        z_kmpkg_find_acquire_program_find_external("${program}"
            ${extra_search_args}
    ...
    if(NOT ${program})
        if("${download_urls}" STREQUAL "" AND "${sourceforge_args}" STREQUAL "")
            set(example ".")
            if(NOT "${brew_package_name}" STREQUAL "" AND KMPKG_HOST_IS_OSX)
                set(example ":\n    brew install ${brew_package_name}")
            elseif(NOT "${apt_package_name}" STREQUAL "" AND KMPKG_HOST_IS_LINUX)
        ...
        if("${sourceforge_args}" STREQUAL "")
            ...
            kmpkg_download_sourceforge(archive_path
                ${sourceforge_args}
                SHA512 "${download_sha512}"
                FILENAME "${download_filename}"
            )
        ...
        elseif(tool_subdirectory STREQUAL "")
            ...
            kmpkg_extract_archive(ARCHIVE "${archive_path}" DESTINATION "${full_subdirectory}.temp")
            file(COPY "${full_subdirectory}.temp/" DESTINATION "${full_subdirectory}")
            file(REMOVE_RECURSE "${full_subdirectory}.temp")
        else()
            kmpkg_extract_archive(ARCHIVE "${archive_path}" DESTINATION "${full_subdirectory}")
        endif()
