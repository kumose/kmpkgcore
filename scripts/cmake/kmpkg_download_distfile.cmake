function(kmpkg_download_distfile out_var)
    ...
    if(NOT DEFINED arg_URLS)
        message(FATAL_ERROR "kmpkg_download_distfile requires a URLS argument.")
    endif()
    if(NOT DEFINED arg_FILENAME)
        message(FATAL_ERROR "kmpkg_download_distfile requires a FILENAME argument.")
    endif()
    if(arg_SILENT_EXIT)
    ...
    if(NOT arg_SKIP_SHA512 AND NOT DEFINED arg_SHA512)
        message(FATAL_ERROR "kmpkg_download_distfile requires a SHA512 argument.
If you do not know the SHA512, add it as 'SHA512 0' and retry.")
    ...
    if(_KMPKG_INTERNAL_NO_HASH_CHECK)
    ...
    if(EXISTS "${downloaded_file_path}")
        if(arg_SKIP_SHA512)
            if(NOT arg_ALWAYS_REDOWNLOAD)
                if(NOT _KMPKG_INTERNAL_NO_HASH_CHECK)
    ...
    # kmpkg_download_distfile_ALWAYS_REDOWNLOAD only triggers when NOT _KMPKG_NO_DOWNLOADS
    ...
    kmpkg_list(SET params "x-download" "${arg_FILENAME}")
    foreach(url IN LISTS arg_URLS)
        kmpkg_list(APPEND params "--url=${url}")
    endforeach()
    ...
    if(arg_SKIP_SHA512)
        kmpkg_list(APPEND params "--skip-sha512")
    else()
        kmpkg_list(APPEND params "--sha512=${arg_SHA512}")
    endif()

    # Setting WORKING_DIRECTORY and passing the relative FILENAME allows kmpkg x-download to print
    # the full relative path if FILENAME has /s in it.
    kmpkg_execute_in_download_mode(COMMAND "$ENV{KMPKG_COMMAND}" ${params} RESULT_VARIABLE error_code WORKING_DIRECTORY "${DOWNLOADS}")
