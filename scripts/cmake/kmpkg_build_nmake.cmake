function(kmpkg_build_nmake)
    ...
    if(NOT DEFINED arg_TARGET)
        kmpkg_list(SET arg_TARGET all)
    endif()
    if(arg_ENABLE_INSTALL)
        kmpkg_list(APPEND arg_TARGET install)
    endif()
    ...
    # Set make options
    kmpkg_list(SET make_opts_base /NOLOGO /G /U /F "${arg_PROJECT_NAME}" ${arg_TARGET})

    if(arg_PREFER_JOM AND KMPKG_CONCURRENCY GREATER "1")
        kmpkg_find_acquire_program(JOM)
        get_filename_component(JOM_EXE_PATH "${JOM}" DIRECTORY)
        kmpkg_add_to_path("${JOM_EXE_PATH}")
        if(arg_CL_LANGUAGE AND "${KMPKG_DETECTED_CMAKE_${arg_CL_LANGUAGE}_COMPILER_ID}" STREQUAL "MSVC")
            string(REGEX REPLACE " [/-]MP[0-9]* " " " KMPKG_DETECTED_CMAKE_${arg_CL_LANGUAGE}_FLAGS_DEBUG " ${KMPKG_DETECTED_CMAKE_${arg_CL_LANGUAGE}_FLAGS_DEBUG} /FS")
            string(REGEX REPLACE " [/-]MP[0-9]* " " " KMPKG_DETECTED_CMAKE_${arg_CL_LANGUAGE}_FLAGS_RELEASE " ${KMPKG_DETECTED_CMAKE_${arg_CL_LANGUAGE}_FLAGS_RELEASE} /FS")
        endif()
    else()
    ...
    kmpkg_backup_env_variables(VARS _CL_ LINK)
    ...
    foreach(build_type IN ITEMS debug release)
        if(NOT DEFINED KMPKG_BUILD_TYPE OR KMPKG_BUILD_TYPE STREQUAL build_type)
            if(build_type STREQUAL "debug")
                ...
                if (arg_ENABLE_INSTALL)
                    kmpkg_list(APPEND make_opts "INSTALLDIR=${install_dir_native}\\debug")
                endif()
                kmpkg_list(APPEND make_opts ${arg_OPTIONS} ${arg_OPTIONS_DEBUG})
                if(NOT arg_CL_LANGUAGE STREQUAL "NONE")
                    set(ENV{_CL_} "${KMPKG_DETECTED_CMAKE_${arg_CL_LANGUAGE}_FLAGS_DEBUG}")
                endif()
                set(ENV{_LINK_} "${KMPKG_DETECTED_CMAKE_SHARED_LINKER_FLAGS_DEBUG}")
                ...
                set(make_opts "${make_opts_base}")
                if (arg_ENABLE_INSTALL)
                    kmpkg_list(APPEND make_opts "INSTALLDIR=${install_dir_native}")
                endif()
                kmpkg_list(APPEND make_opts ${arg_OPTIONS} ${arg_OPTIONS_RELEASE})

                if(NOT arg_CL_LANGUAGE STREQUAL "NONE")
                    set(ENV{_CL_} "${KMPKG_DETECTED_CMAKE_${arg_CL_LANGUAGE}_FLAGS_RELEASE}")
                endif()
                set(ENV{_LINK_} "${KMPKG_DETECTED_CMAKE_SHARED_LINKER_FLAGS_RELEASE}")
            ...
            if(arg_PREFER_JOM)
                execute_process(
                    COMMAND "${JOM}" /K /J ${KMPKG_CONCURRENCY} ${make_opts}
                    WORKING_DIRECTORY "${object_dir}${project_subpath}"
            ...
            kmpkg_restore_env_variables(VARS _CL_ LINK)
        endif()
    endforeach()
endfunction()
